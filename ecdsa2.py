...
class ECDSA2():
    ...
    def __init__(self, params: ECDSA2_Params):
    ...
    def KeyGen(self) -> Tuple[IntegerMod_int, Point]:
        ...
        sk = self.Z_q(secrets.SystemRandom().randrange(1, self.q))

        # add point in cycle to each other
    ...
    def Sign(self, privkey, msg) -> Tuple[IntegerMod_int, IntegerMod_int]:
        ...
        k = self.Z_q(secrets.SystemRandom().randrange(1, self.q))
        rng_p = self.P.copy() * k
        r = self.Z_q(rng_p.x)
        inverted_k = self.Z_q(1) / k
        s = self.Z_q(inverted_k * (h + self.Z_q(privkey * r)))
        return r, s
...
if __name__ == "__main__":
    ...
    print("Verify")
    assert ecdsa2.Verify(Q, msg, r, s)
